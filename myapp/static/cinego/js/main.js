(function (window, document, undefined) {
	...
	Regula
	==============================*/
	if (document.querySelector('[data-constraints]')) {
		const elements = document.querySelectorAll('[data-constraints]');
		const submitButton = document.querySelector('button[type="submit"]'); // Assuming your form has a submit button
	
		// Attach validation to form elements
		function attachFormValidator(elements) {
			elements.forEach((element) => {
				// Add necessary validation UI elements
				element.classList.add("form-control-has-validation");
				const validationSpan = document.createElement('span');
				validationSpan.classList.add('message__text');
				element.parentNode.insertBefore(validationSpan, element.nextSibling);
		...
		function handleValidation(event) {
			...
			if (element.dataset.constraints.includes('@PhoneNumber')) {
				if (!validatePhoneNumber(value)) {
					validationSpan.textContent = 'Invalid phone number format';
			...
			if (element.dataset.constraints.includes('@Required')) {
				if (!value) {
					validationSpan.textContent = 'The field is required.';
					element.parentElement.classList.add('has-error');
				} else {
					validationSpan.textContent = '';
					element.parentElement.classList.remove('has-error');
			...
			if (element.dataset.constraints.includes('@Email')) {
				const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
				if (!emailPattern.test(value)) {
					validationSpan.textContent = 'The email is not a valid email.';
					element.parentElement.classList.add('has-error');
				} else {
					validationSpan.textContent = '';
					element.parentElement.classList.remove('has-error');
		...
		// Ensure validation when the submit button is clicked
		function validateOnSubmit() {
			...
			elements.forEach((element) => {
				...
				if (element.dataset.constraints.includes('@Required') && !value) {
					validationSpan.textContent = 'The field is required.';
					element.parentElement.classList.add('has-error');
					allValid = false;
				} else if (element.dataset.constraints.includes('@PhoneNumber') && !validatePhoneNumber(value)) {
					validationSpan.textContent = 'Invalid phone number format';
					element.parentElement.classList.add('has-error');
					allValid = false;
				} else if (element.dataset.constraints.includes('@Email') && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
					validationSpan.textContent = 'The email is not a valid email.';
					element.parentElement.classList.add('has-error');
					allValid = false;
				} else {
		...
		// Attach form validation to the elements with data-constraints
	...
	/*==============================
	Forms handling with Ajax 
	==============================*/
	if (document.querySelector('.ajax-form')) {
		// Attach event listener to all forms with the class 'ajax-form'
