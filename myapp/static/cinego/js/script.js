...
(function () {
	...
	$(function () {
		...
		/**
		 * @desc Initialize the gallery with one image or video
		...
		function initLightGalleryItem(itemToInit, addClass) {
			if (!isNoviBuilder) {
				$(itemToInit).lightGallery({
					...
					addClass: addClass,
					counter: false,
					videojs: true, // Ensures support for local video files
					dynamic: false, // Ensures it's not treated as a gallery
					download: false, // Disable download button if not needed
					enableDrag: false, // Disable dragging (optional)
					autoplayFirstVideo: true, // Autoplay the first video (optional)
					videojs: true, // For local video support
					mode: 'lg-fade', // Use fade transition for a smooth video opening
					loop: false, // Prevent looping the video
					share: false, // Disable sharing options (optional)
				});
			}
		}
		...
		// Global AJAX Handler
		if (plugins.ajaxForm.length) {
			plugins.ajaxForm.on('submit', function (e) {
				e.preventDefault();
	
				var $form = $(this);
				var formData = new FormData(this);
				formData.append('csrfmiddlewaretoken', $('[name="csrfmiddlewaretoken"]').val()); // Add CSRF token
	
				// Send AJAX request inside the if statement
				function sendAjaxRequest($form, formData) {
					$.ajax({
				...
				sendAjaxRequest($form, formData); // Call the sendAjaxRequest function
			});
